...
- **`http`** — the server binds to a port (`PRACTICE_HTTP_HOST` / `PRACTICE_HTTP_PORT`, default `127.0.0.1:7180`) and runs as a long-lived process serving many clients. Active practice state is scoped per MCP session (a `ContextVar` + `_SessionState`), so concurrent HTTP clients are isolated and no longer race; the former `PRACTICE_EXPERIMENTAL_HTTP` gate is no longer required.
...
# HTTP — long-lived process serving many clients; per-session state isolates them
...
**One thing to know before any of these commands.** No router sends every closed enactment to `judge_inbox` — closed enactments pass through deterministic triage first, and only the *ambiguous* are queued for the Judge. Two routers apply that gate on different cadences: the dispatcher triages somatic completions continuously, and a separate reflective loop in the runner triages autonomic history (the Judge's and Smoother's own enactments) on a slower watermarked pass. Because both go through the gate, a Judge enactment that emits zero Friction is classified clean (a no-finding) and is not requeued, so the loop converges instead of judging the Judge's judgement of the Judge indefinitely. Step 12 ("One mechanism, two cadences") walks through how triage closes the convergence gap earlier drafts left open. The commands below run the same mechanism against a real LLM; the deterministic gate keeps the autonomic loop from spending tokens on clean work. Worth knowing before you point one of these at a paid API.

**`AnthropicSDKAdapter` — long-lived in-process.** The SDK keeps a long-lived `ClaudeSDKClient` per role. The MCP transport is configurable: by default (`PRACTICE_AUTONOMIC_MCP_URL` unset) each adapter instance spawns its own stdio MCP server subprocess. With `PRACTICE_AUTONOMIC_MCP_URL` set, the adapter connects to a long-lived HTTP server. Active practice state is scoped per MCP session, so that HTTP server is concurrency-safe; the former `PRACTICE_EXPERIMENTAL_HTTP` gate is no longer required. Stdio default:
...
HTTP shape (long-lived server; per-session state isolates concurrent clients):
...
If you prefer to point it at an existing long-lived HTTP server instead, start that server (`PRACTICE_TRANSPORT=http`) and set `PRACTICE_AUTONOMIC_MCP_URL=http://…/mcp/`; the adapter switches its `--mcp-config` shape accordingly.
...
For the somatic side, the user's harness connects to the somatic server the same way: Codex via the `.mcp.json` entry `apprenticeship_somatic`, Claude Code via its MCP-server config pointing at an HTTP somatic server (`PRACTICE_TRANSPORT=http PRACTICE_SERVER_MODE=somatic`).
...
The verify uses `ScriptedAdapter` end-to-end and prints, in order: the somatic walk (engagement projected, five practices enacted — Activities Management, Reflection, Calendar Stewardship, Practice Management, and the runtime-authored Quick Glance), the triaged inbox counts after the somatic side closes (only the ambiguous reach the Judge; deterministic Frictions go straight to the Smoother), the Judge drain examining the ambiguous enactment, the Smoother drain consuming the Frictions triage emitted, the trail showing every enactment top-level or nested, and the Friction summary moving each observation from pending to addressed.
...
The autonomic loop now runs through one revolution — the user's somatic work generates closed enactments, the dispatcher routes them, the Judge enacts and emits Friction, the dispatcher routes the Friction, the Smoother enacts and amends. What is still left implicit is that **the Judge's enactments and the Smoother's enactments are themselves recorded in the trail**, alongside any other enactment. The same deterministic triage examines them too. The Judge can examine a Judge enactment; the Smoother can amend Smoother's own bundle. Step 12 names this — the strange loop the second essay described.

## Step 12: The strange loop

Everything the loop touches gets examined the same way. A somatic enactment closes — deterministic triage classifies it, and an ambiguous one is routed to `judge_inbox`. A Judge enactment closes — the same triage classifies it too, with the same code, against the same trail. There is no special case: the Judge bundle is a bundle, its enactments are enactments, and triage has no idea what kind of work it is classifying. What differs from a naive route is that triage only queues the *ambiguous*. A clean Judge enactment — one that read its target, used its affordances, and recorded no error — is cleared as a deterministic no-finding, never queued, never charged to the LLM.
...
Essay 2 named the runaway-recursion fix as **two loops**: a reactive loop responding to somatic completions, and a separately-scheduled reflective loop examining autonomic history. It said, plainly, that *when an autonomic enactment completes, no notification is dispatched*; the autonomic history was to be visited only on the reflective loop's own schedule. The implementation recovers those two loops as **two triage modes over one idempotent inbox**. `triage_and_route(mode="somatic")` runs continuously in the dispatcher, responding to somatic completions; `triage_and_route(mode="autonomic")` runs on the autonomic runner's own slower cadence, examining autonomic history. Both feed the same `judge_inbox`, but neither routes blindly — each closed enactment passes the deterministic gate first, and only the ambiguous are queued.
...
Earlier drafts of this implementation left a real convergence gap here. `judge_inbox` was fed by *closed enactments*, not by Friction; so a Judge enactment that emitted zero Friction still closed, still got routed, and still became another Judge work item. A continuously-polling dispatcher with no other gate would judge the Judge's judgement of the Judge, indefinitely, even when each pass produced nothing. Quieting the smoother_inbox did not quiet judge_inbox.

Deterministic triage closes that gap directly. A Judge enactment that records no error and leaves its target properly read is classified clean and is *not* requeued; only the ambiguous spawn a fresh Judge work item, and a clean judgement of an ambiguous case is itself triaged clean and stops there. The self-examination still happens when there is something to examine — it no longer manufactures work out of its own quiet. The shape essay 2's two-loop instinct pointed at ended up combining both of its moves: the cadence split is real (somatic triaged continuously by the dispatcher, autonomic on the runner's slower pass), and the per-enactment gate is real too (triage filters routing so a no-Friction Judge enactment is never requeued). Recovered not as a second loop but as a discipline at the routing layer — which is where the determinable/judgement boundary belongs.

### Demonstrating one revolution

The verify, after running the first pass of Judge and Smoother, calls `route_now(store)` again. The Judge and Smoother enactments produced by the first pass have closed; triage examines each one. Because they read their targets, used multiple affordances, and recorded no error, every one is classified clean — a deterministic no-finding. None is added to `judge_inbox`, so the second `drain` of the Judge finds nothing to do. The strange loop quiets before the LLM is consulted a second time.
...
```text
First pass:
  dispatcher (one-shot): judge_inbox +1, smoother_inbox +2
  pending judge_inbox  : 1         # one ambiguous enactment (a recorded step error)
  Judge drained 1 work item(s)
  pending smoother_inbox: 2        # two deterministic missing_bundle Frictions
  Smoother drained 2 work item(s)

Second pass (strange loop — Judge examines Judge/Smoother enactments):
  dispatcher (one-shot): judge_inbox +0, smoother_inbox +0
  pending judge_inbox  : 0         # pass-1 autonomic enactments all triaged clean
  Judge drained 0 work item(s) on the second pass
```

On the first pass, triage queues exactly one of the six somatic enactments to the Judge: the engagement enactment, whose memory-recall step recorded an error (`{"error": ...}`) because the hermetic verify has no Neo4j auth — a recorded step error is the canonical ambiguous signal, the one case that genuinely needs the Judge to name what went wrong. The other five enactments are deterministic no-findings. Separately, triage emits two provable `missing_bundle` Frictions — somatic enactments tagged to bundles that do not resolve in the loaded catalog — and routes them straight to the Smoother, no Judge dispatch. The Judge drains its one item and emits no Friction; the Smoother addresses the two.

On the second pass, zero new judge_inbox rows. The five Judge enactments and the Smoother enactment from pass 1 all closed cleanly — each read its target and used at least two affordances (`read_enactment_steps` plus `read_bundle`) with no recorded error — so triage classified each as a deterministic no-finding and requeued none. The loop went still without a single second-pass Judge dispatch; the LLM was never asked to confirm what the deterministic gate could already settle.

That stillness matters, and triage moved its source. In the original loop the pass quieted only because the Judge's discipline — observe, do not invent — found nothing worth naming; the loop still spent a dispatch per enactment to discover that. Now the deterministic gate quiets the clean cases for free, and the Judge's discipline is reserved for the genuinely ambiguous ones that reach it. The determinable/judgement boundary became the code/LLM boundary: clean work is settled in code, and the LLM is asked only where judgement is actually required.
...
In production with real LLM enactment, the run-loop variant (`run_role_loop`) is used instead — it polls forever, idle-waits when empty, and stops on a signal. Two routers keep the inboxes populated, and the convergence comes from their split, not from one continuously-polling dispatcher. The dispatcher triages *somatic* completions continuously (`triage_and_route(mode="somatic")`); a separate reflective loop in the runner (`_run_reflective_judge_loop`) triages *autonomic* history on a slower, watermarked cadence (`triage_and_route(mode="autonomic")`), so the Judge's own enactments are examined in their own time rather than on every poll. Both go through the same deterministic gate, so an autonomic enactment that emits no Friction is classified clean and does not re-enter `judge_inbox` — the reflective loop quiets instead of feeding itself.
