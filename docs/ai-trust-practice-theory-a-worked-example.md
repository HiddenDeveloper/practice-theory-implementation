...
A small worked-example aside before the close. The first real-LLM run of this case study — Codex driving the somatic server, the prompt simply "I have a conflict with the Acme customer review on Friday at 2pm; can you push it back an hour?" — surfaced a gap in the bundle that the architecture itself made inspectable. Codex switched into the practice and tried to call the calendar's listing material with the parameter names `start` and `end`. The material's actual schema declares `start_date` and `end_date`; the LLM had to guess. The reason the guess was needed: `discover_affordances` returned material *names* but not their *schemas*, so the harness LLM saw the affordance existed but had to confabulate the argument shape on first try. The fix — surface each material's `input_schema` inline in the `discover_affordances` result — is the kind of small architectural correction the trail-as-evidence loop is built to surface. The Calendar Stewardship case study did not just exercise the bundle; it exercised the *surface*, and one round of use closed a small gap the verify hadn't shown. The same loop story applies to the bundle's own missing-attendee-consent step. Practices improve through use; the architecture makes the improvement legible.
