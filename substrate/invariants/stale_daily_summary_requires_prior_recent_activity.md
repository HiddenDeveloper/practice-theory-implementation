...
---
Deterministic guard for the stale daily-summary branch of the `activities_management` quality coverage contract named by Frictions 716 and 726. Existing guards cover valid `garmin_get_daily_summary` reads and invalid stale-material reaches separately; this guard covers the mechanically checkable coverage fact when a stale `garmin_daily_summary` attempt occurs before any `garmin_list_activities` activity-record grounding and routes it as `quality_affordance_coverage`. Judgement about isolated daily-snapshot exceptions or explicit list/auth/data/material blockers remains with practice evaluation and the Judge.
