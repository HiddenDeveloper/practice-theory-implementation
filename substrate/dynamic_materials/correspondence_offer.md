...
Friction 738 confirms the remaining miss is still occurring at the post-search/access branch even though nearby correspondent traces show `gmail_user_get_thread` is reachable. When a correspondent pass has reached live Gmail search, test-search recovery, or a Gmail auth/credential/access error and no selected user thread has been retrieved, this material must be treated as an interruption gate rather than a way to keep moving. The next offering must either be preceded by `read_user_email` reaching `gmail_user_get_thread` for the selected thread, or be a `kind: "limit"` / explicitly limited artifact that names the exact search/auth/credential/access/retrieval gap and says the work is confined to supplied text or visible search metadata. Do not use this material for warm stance invitations, thread analysis, draft language, or final reply wording from search-only, test-search-only, or auth-error evidence.
