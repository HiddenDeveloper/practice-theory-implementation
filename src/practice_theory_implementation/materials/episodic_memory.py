...
def recall_relevant_episodes(
    ...
    Returns the store's vector-ranked hits as-is (most similar first), each with
    its native similarity `score`, narrowed only by the filters and
    `score_threshold` the caller asks for. This material does not re-rank or
    re-weight the hits: judging which recalled turns matter — and why — is the
    practitioner's situated call, not a heuristic baked in here.
    """
    if not query.strip():
